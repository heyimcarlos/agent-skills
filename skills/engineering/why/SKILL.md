...
Before spawning investigators, inspect the available tools and connectors in the Codex environment. Use the available-tools map when present. If the environment supports lazy tool discovery, search it for sources relevant to each evidence category.
...
Call Codex's `spawn_agent` once per matching investigator without waiting between calls so they run concurrently. One investigator per category lets each specialize in one tool's query vocabulary and result shape. Don't ask one agent to cover multiple MCPs.

Subagent config (each):
- Use Codex's `spawn_agent` tool.
- Inherit the parent model unless the current Codex collaboration environment exposes a configured investigator model.
- Give the investigator an explicit read-only task contract. It may use connected tools to search, but it must not write files or mutate external systems. That's a posture, not a sandbox.
...
Spawn one synthesizer subagent with Codex's `spawn_agent` tool:

- Prefer a different model family from the parent's and the investigators when one is available. Otherwise inherit the parent model.
- Give the synthesizer an explicit read-only task contract. It may use connected tools to spot-check citations, but it must not write files or mutate external systems.
