...
2a. read `~/scripts/create_worktree.sh` and create a new worktree with the action for the implementation (feature, bug, hotfix) branch name: `~/scripts/create_worktree.sh action/<short-desc> BRANCH_NAME`
...
worktree path: ~/wt/<repo-name>/<action>
...
command to run:
    ...
    humanlayer launch --model opus -w ~/wt/<repo-name>/<action> "/implement_plan at $FILEPATH and when you are done implementing and all tests pass, read ./claude/commands/commit.md and create a commit, then read ./claude/commands/describe_pr.md and create a PR, then add a comment to the Linear ticket with the PR link"
```
