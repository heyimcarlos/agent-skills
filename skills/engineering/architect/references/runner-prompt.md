...
- Shared state: if two actors might both write, ask "what happens?" If the answer isn't "nothing," default to per-actor state with a merge at the read boundary, per the [**separate-before-serializing-shared-state** principle skill](../../principle-separate-before-serializing-shared-state/SKILL.md).
...
- Encode invariants in types: hard-to-misuse types > runtime checks > prose comments, per the [**encode-lessons-in-structure** principle skill](../../principle-encode-lessons-in-structure/SKILL.md).
- Validate at boundaries, trust types inside, per the [**boundary-discipline** principle skill](../../principle-boundary-discipline/SKILL.md). Business logic as pure functions; the shell stays thin.
- Single source of truth per invariant. Derive instead of sync.
- Idempotent state transitions where applicable, per the [**make-operations-idempotent** principle skill](../../principle-make-operations-idempotent/SKILL.md). Ask what happens if the operation runs twice or crashes halfway.
- Short call chains. If tracing the flow needs more than three files, flatten the hierarchy, per the [**laziness-protocol**](../../principle-laziness-protocol/SKILL.md) and [**minimize-reader-load**](../../principle-minimize-reader-load/SKILL.md) principle skills.

You are one of several runners. Use a different available model variant when possible. Produce the best design your model can make; don't hedge against the others. Differences between candidates are the signal used to pick a base and graft. Converging on a safe-looking middle defeats the exploration.
