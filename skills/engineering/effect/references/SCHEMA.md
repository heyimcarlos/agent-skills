...
- Use `Schema.Class` or `Schema.TaggedClass` when validated construction, methods, or class identity justify them; keep structural schemas as the default for plain data.

## Errors

Use `Schema.TaggedError` for typed Effect errors that need a schema.
...
- Use `Data.TaggedError` for internal errors that need no codec or schema metadata.
- Use `Result` from `effect` when synchronous success or failure should remain data.
