...
- Model schema-backed Effect errors with `Schema.TaggedError`; use `Data.TaggedError` for internal errors that need no schema.
- Use the Effect error channel for workflow failures and `Result` from `effect` when synchronous failure should remain data.
...
- Schema-backed expected failure: `Schema.TaggedError`.
- Internal expected failure with no schema needs: `Data.TaggedError`.
- Synchronous success or failure retained as data: `Result` from `effect`.
...
- Use `Schema.Class` or `Schema.TaggedClass` when validated construction, methods, or class identity justify them; keep `Schema.Struct` as the default for plain records.
- Do not hand-roll `_tag` error classes when `Schema.TaggedError` or `Data.TaggedError` fits.
